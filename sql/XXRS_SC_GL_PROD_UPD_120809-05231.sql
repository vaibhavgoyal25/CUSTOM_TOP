/**************************************************************************************************************
*                                                                                                             *
* NAME : XXRS_SC_GL_PROD_UPD_120809-05231.sql                                                                 *
*                                                                                                             *
* DESCRIPTION :                                                                                               *
...
***************************************************************************************************************/
--
/* $Header: XXRS_SC_GL_PROD_UPD_120809-05231.sql 1.0.0 09/21/2012 11:06:24 AM Pavan Amirineni $ */
--     
...
SELECT 'XXRS_SC_GL_PROD_UPD_120809-05231_'||TO_CHAR(SYSDATE,'YYYYMMDDHHMISS')||'.log' file_name
FROM DUAL;

SPOOL &spool_file_name
                         
DECLARE 
CURSOR UPD_DEV_PRD IS
...
--   AND ROWNUM < 1
  ...
  BEGIN 
--      
    FOR rec_upd_dev_prd IN upd_dev_prd
    LOOP 
    
      UPDATE xxrs.xxrs_sc_device_product_tbl 
         SET service_id = rec_upd_dev_prd.service_id
           , last_update_date = SYSDATE
           , last_updated_by  = 0 
       WHERE device_product_snid = rec_upd_dev_prd.device_product_snid
         AND service_id IS NULL;                  
    END LOOP; 
    
 --   COMMIT;
         
    FOR rec_upd_dev_prd_ser IN cur_upd_dev_prd_ser
    LOOP
      UPDATE xxrs.xxrs_sc_device_resource_tbl 
         SET service_id = rec_upd_dev_prd_ser.service_id
           , last_update_date = SYSDATE
           , last_updated_by  = 0 
       WHERE device_resource_snid = rec_upd_dev_prd_ser.device_resource_snid
         AND service_id IS NULL;         
    END LOOP;
--  
 --   COMMIT;   
    FOR rec_upd_act_prd_ser IN cur_upd_act_prd_ser
    LOOP
      UPDATE xxrs.xxrs_sc_account_resource_tbl
         SET service_id = rec_upd_act_prd_ser.service_id
           , last_update_date = SYSDATE
           , last_updated_by  = 0 
       WHERE account_resource_snid = rec_upd_act_prd_ser.account_resource_snid
         AND service_id IS NULL;
    END LOOP;            
--
 --   COMMIT;     
  END;   
/
PROMPT "Please issue the commit"
