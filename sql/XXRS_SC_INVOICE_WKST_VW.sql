/********************************************************************************************************************
* NAME : XXRS_SC_INVOICE_WKST_VW.sql                                                                                *
*                                                                                                                   *
* DESCRIPTION :                                                                                                     *
*   View developed for Invoice worksheet form                                                                       *
*                                                                                                                   *
* AUTHOR       : Kalyan                                                                                             *
...
********************************************************************************************************************/
--
/* $Header: XXRS_SC_INVOICE_WKST_VW.sql 1.0.1 02-SEP-2013 10:00:00 AM Mahesh Guddeti $ */

DROP VIEW APPS.XXRS_SC_INVOICE_WKST_VW;

CREATE OR REPLACE FORCE VIEW APPS.XXRS_SC_INVOICE_WKST_VW
(ROW_ID, CREATED_BY_USER, SITE_NUM, ADDRESS, STATUS_MEANING, 
 ...
 INV_LINE_NUM, CUSTOMER_TRX_LINE_ID, MEMO_LINE_ID, MEMO_LINE,FILE_NAME)
AS 
SELECT iw.ROWID row_id
       ...
       , ml_tl.name                                         -- LY 110223-01761
       , iw.file_name   -- ADDED BY MAHESH GUDDETI ON 02 SEP 2013 --130617-06898
    FROM xxrs_sc_invoice_wkst_int_tbl iw
       , xxrs_sc_cust_bill_to_sites_vw cus
       , hz_party_sites party_site
       , fnd_user usr
       , fnd_lookup_values flv
       , fnd_lookup_types flt
       , ar.ar_memo_lines_all_tl ml_tl
       , xxrs.xxrs_sc_service_def srvc_def
   WHERE party_site.party_site_id = cus.party_site_id
     ...
     AND ml_tl.memo_line_id(+) = iw.memo_line_id
     AND ml_tl.org_id(+) = iw.org_id
     AND srvc_def.service_id(+) = iw.service_id
     AND NVL ( iw.org_id
...
AUDIT GRANT ON APPS.XXRS_SC_INVOICE_WKST_VW BY SESSION WHENEVER SUCCESSFUL;

AUDIT GRANT ON APPS.XXRS_SC_INVOICE_WKST_VW BY SESSION WHENEVER NOT SUCCESSFUL;
